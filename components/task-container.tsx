...
const TaskContainer = ({heading}:TaskContainerProps) => {
    const queryClient = useQueryClient();
    // filtering task based on status
    const [fetchingProgress, setFetchingProgress] = useState(0);
    const [page, setPage] = useState(0);
    const [fetchedAllTask, setFetchAllTasks] = useState<Task[] | undefined>([]);

    const {data, error, isLoading} = useQuery({ queryKey: ['getTasks', heading, page], queryFn: () => fetchAllTasks(setFetchingProgress, page, heading) })
  ...
  return (
    <>  
        ...
        <h2 className='text-xs font-bold uppercase'>{heading} <span className='bg-gray-300 p-1 px-2 rounded-sm'>{fetchedAllTask?.length || 0}</span></h2>
